import os, struct, csv, random, subprocess, collections
import numpy as np
import cocotb
...
def gen_random_vectors(n: int = 200, seed: int = 123):
    ...
    edges = [
        ...
        (mk_inf(0),mk_inf(1)),
        (mk_inf(0),0x00000000),
        (mk_qnan(1), mk_norm(0,120,1)),
        (mk_sub(0,1), mk_norm(0,127,0)),
        (mk_norm(0,254,0x7FFFFF), 0x40000000),
    ]
...
# ---------- cocotb test: FP32 MUL (pipelined) ----------
@cocotb.test()
async def check_fpu_mul_riscv(dut):
    ...
    tf.close(); cf.close()
    dut._log.info("Wrote human-readable log: results_table.log")
    dut._log.info("Wrote CSV: results.csv")
